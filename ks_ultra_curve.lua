...
Onyx.Appearance = {
    ...
    KeylessTitle = "Pandu Hub",
    KeylessSubtitle = "No key required for this build - you're verified.",
    Icon = "rbxassetid://109651670989001",
}

Onyx.Links = {
    GetKey = "https://discord.gg/panduhub",
    Discord = "https://discord.gg/panduhub",
}

Onyx.Storage = {
    FileName = "PanduHub_key",
    Remember = true,
    AutoLoad = true,
}
...
Onyx.Shop = {
    ...
    ButtonText = "Buy",
    Link = "https://discord.gg/panduhub"
}
...
local function PCUltraCurveUpdateButton()
    local button = PCUltraCurve.ToggleButton
    if not button then return end

    if PCUltraCurve.Enabled then
        button.Text = "PC Ultra Curve: ON"
        button.BackgroundColor3 = Color3.fromRGB(255, 100, 150)
    else
        button.Text = "PC Ultra Curve: OFF"
        button.BackgroundColor3 = Color3.fromRGB(30, 24, 45)
    end
end

local function PCUltraCurveStop()
    PCUltraCurve.Enabled = false

    if PCUltraCurve.Connection then
        PCUltraCurve.Connection:Disconnect()
        PCUltraCurve.Connection = nil
    end

    local camera = workspace.CurrentCamera
    if camera then
        PCUltraCurve.CurrentCFrame = camera.CFrame
        PCUltraCurve.LastLookVector = camera.CFrame.LookVector
    end

    PCUltraCurve.ArcVelocity = Vector3.new(0, 0, 0)
    PCUltraCurveUpdateButton()
end

local function PCUltraCurveStart()
    if PCUltraCurve.Connection then
        PCUltraCurve.Connection:Disconnect()
        PCUltraCurve.Connection = nil
    end
    ...
    PCUltraCurve.Enabled = true
    PCUltraCurve.CurrentCFrame = camera.CFrame
    PCUltraCurve.LastLookVector = camera.CFrame.LookVector
    PCUltraCurve.ArcVelocity = Vector3.new(0, 0, 0)

    PCUltraCurve.Connection = RunService.RenderStepped:Connect(function()
        if not PCUltraCurve.Enabled then return end
        ...
        local lastLook = PCUltraCurve.LastLookVector or currentLook
        local rightVector = targetCFrame.RightVector
        local upVector = targetCFrame.UpVector

        local turnDot = math.clamp(currentLook:Dot(lastLook), -1, 1)
        local turnCross = lastLook:Cross(currentLook)
        local turnAmount = math.clamp(turnCross:Dot(upVector) * 8, -1, 1)
        local tiltFactor = 0

        -- Sangat sensitif terhadap perubahan arah kamera.
        if turnDot < 0.98 then
            tiltFactor = math.clamp(
                turnAmount * PCUltraCurve.TiltAmount,
                -PCUltraCurve.TiltAmount,
                PCUltraCurve.TiltAmount
            )

            PCUltraCurve.ArcVelocity =
                PCUltraCurve.ArcVelocity
                + (rightVector * (turnAmount * PCUltraCurve.ArcStrength))
                + (currentLook * PCUltraCurve.Overshoot * math.abs(turnAmount))
        end

        -- Redaman lambat untuk mempertahankan efek lengkungan.
        PCUltraCurve.ArcVelocity = PCUltraCurve.ArcVelocity:Lerp(
            Vector3.new(0, 0, 0),
            0.06
        )

        PCUltraCurve.CurrentCFrame = PCUltraCurve.CurrentCFrame:Lerp(
            targetCFrame,
            PCUltraCurve.Smoothness
        )

        currentCamera.CFrame =
            (PCUltraCurve.CurrentCFrame + PCUltraCurve.ArcVelocity)
            * CFrame.Angles(0, 0, tiltFactor)

        PCUltraCurve.LastLookVector = currentLook
    end)
...
local function PCUltraCurveSet(value)
    if value then
        return PCUltraCurveStart()
    end
...
local function PCUltraCurveCreateGui()
    if PCUltraCurve.Gui then
        PCUltraCurve.Gui:Destroy()
    end

    local gui = Instance.new("ScreenGui")
    gui.Name = "PanduHub_PC_UltraCurve"
    gui.ResetOnSpawn = false
    gui.IgnoreGuiInset = true
    gui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
    gui.Parent = CoreGui

    PCUltraCurve.Gui = gui

    local toggleButton = Instance.new("TextButton")
    toggleButton.Name = "CurveToggle"
    toggleButton.Size = UDim2.fromOffset(180, 40)
    ...
    PCUltraCurve.ToggleButton = toggleButton
    PCUltraCurve.HideButton = hideButton

    toggleButton.MouseButton1Click:Connect(function()
        local newValue = not PCUltraCurve.Enabled
        PCUltraCurveSet(newValue)
    end)

    hideButton.MouseButton1Click:Connect(function()
        toggleButton.Visible = not toggleButton.Visible
        hideButton.Text = toggleButton.Visible and "HIDE" or "SHOW"
    end)

    PCUltraCurveUpdateButton()
end

local function PCUltraCurveDestroyGui()
    PCUltraCurveStop()
    ...
    PCUltraCurve.Gui = nil
    PCUltraCurve.ToggleButton = nil
    PCUltraCurve.HideButton = nil
end

FakePerksGroup:AddButton({
    Text = "PC Ultra Curve",
    Tooltip = "Munculkan tombol PC Ultra Curve di layar",
    Func = function()
        if PCUltraCurve.Gui then
            PCUltraCurveDestroyGui()
            Library:Notify({
                Title = "PC Ultra Curve",
                Description = "Button disembunyikan",
                Time = 2
            })
        else
            PCUltraCurveCreateGui()
            Library:Notify({
                Title = "PC Ultra Curve",
                Description = "Button muncul!",
                Time = 2
            })
        end
    end
