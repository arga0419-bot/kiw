...
-- Purple color scheme
Library.Scheme.AccentColor     = Color3.fromRGB(150, 90, 255)
Library.Scheme.BackgroundColor = Color3.fromRGB(15, 15, 20)
Library.Scheme.MainColor       = Color3.fromRGB(25, 25, 25)
Library.Scheme.OutlineColor    = Color3.fromRGB(90, 70, 140)
...
local function UpdateCurveButton()
    if not ExtremeCurve.ToggleButton then return end

    if ExtremeCurve.Enabled then
        ExtremeCurve.ToggleButton.Text = "PC Heavy Curve: ON"
        ExtremeCurve.ToggleButton.BackgroundColor3 = Color3.fromRGB(150, 100, 255)
    else
        ExtremeCurve.ToggleButton.Text = "PC Heavy Curve: OFF"
        ExtremeCurve.ToggleButton.BackgroundColor3 = Color3.fromRGB(30, 24, 45)
    end
end

local function StopExtremeCurve()
    ExtremeCurve.Enabled = false

    if ExtremeCurve.Connection then
        ExtremeCurve.Connection:Disconnect()
        ExtremeCurve.Connection = nil
    end

    local cam = workspace.CurrentCamera
    if cam then
        ExtremeCurve.CurrentCFrame = cam.CFrame
        ExtremeCurve.LastLookVector = cam.CFrame.LookVector
    end

    ExtremeCurve.ArcVelocity = Vector3.new(0, 0, 0)
    UpdateCurveButton()
end

local function StartExtremeCurve()
    if ExtremeCurve.Connection then
        ExtremeCurve.Connection:Disconnect()
        ExtremeCurve.Connection = nil
    end
    ...
    ExtremeCurve.Enabled = true
    ExtremeCurve.CurrentCFrame = cam.CFrame
    ExtremeCurve.LastLookVector = cam.CFrame.LookVector
    ExtremeCurve.ArcVelocity = Vector3.new(0, 0, 0)

    ExtremeCurve.Connection = RunService.RenderStepped:Connect(function()
        if not ExtremeCurve.Enabled then return end

        local currentCamera = workspace.CurrentCamera
        if not currentCamera then return end

        local targetCFrame = currentCamera.CFrame
        local currentLook = targetCFrame.LookVector
        local rightVector = targetCFrame.RightVector
        local lastLook = ExtremeCurve.LastLookVector or currentLook

        local turnDot = currentLook:Dot(lastLook)
        local tiltFactor = 0

        -- Arah belokan dihitung dari perubahan LookVector terhadap kamera sebelumnya.
        local turnCross = lastLook:Cross(currentLook)
        local turnAmount = math.clamp(turnCross:Dot(targetCFrame.UpVector), -1, 1)

        if turnDot < 0.92 then
            tiltFactor = math.clamp(turnAmount * ExtremeCurve.TiltAmount * 4, -ExtremeCurve.TiltAmount, ExtremeCurve.TiltAmount)

            -- Efek arc ke samping ketika kamera berbelok.
            ExtremeCurve.ArcVelocity =
                ExtremeCurve.ArcVelocity
                + (rightVector * (turnAmount * ExtremeCurve.ArcStrength))
        end

        -- Redam momentum supaya tidak terus bergeser.
        ExtremeCurve.ArcVelocity =
            ExtremeCurve.ArcVelocity:Lerp(Vector3.new(0, 0, 0), 0.10)

        ExtremeCurve.CurrentCFrame =
            ExtremeCurve.CurrentCFrame:Lerp(
                targetCFrame,
                ExtremeCurve.Smoothness
            )

        currentCamera.CFrame =
            (ExtremeCurve.CurrentCFrame + ExtremeCurve.ArcVelocity)
            * CFrame.Angles(0, 0, tiltFactor)

        ExtremeCurve.LastLookVector = currentLook
    end)
...
local function SetExtremeCurve(value)
    if value then
        return StartExtremeCurve()
    else
        StopExtremeCurve()
        return true
    end
...
local function DestroyExtremeCurveGui()
    StopExtremeCurve()

    if ExtremeCurve.Gui then
        pcall(function()
            ExtremeCurve.Gui:Destroy()
        end)
    end

    ExtremeCurve.Gui = nil
    ExtremeCurve.ToggleButton = nil
    ExtremeCurve.HideButton = nil
    ExtremeCurve.ButtonVisible = false
end

local function CreateExtremeCurveGui()
    if ExtremeCurve.Gui then
        pcall(function()
            ExtremeCurve.Gui:Destroy()
        end)
    ...
    local gui = Instance.new("ScreenGui")
    gui.Name = "PanduHubExtremeCurve"
    gui.ResetOnSpawn = false
    gui.IgnoreGuiInset = true
    gui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
    gui.Parent = CoreGui

    ExtremeCurve.Gui = gui
    ExtremeCurve.ButtonVisible = true

    -- Tombol PC Heavy Curve
    local toggle = Instance.new("TextButton")
    toggle.Name = "CurveToggle"
    toggle.Size = UDim2.fromOffset(180, 40)
    toggle.Position = UDim2.new(0.5, -90, 0, 60)
    toggle.BackgroundColor3 = Color3.fromRGB(30, 24, 45)
    toggle.TextColor3 = Color3.fromRGB(255, 255, 255)
    toggle.Text = "PC Heavy Curve: OFF"
    toggle.TextSize = 14
    ...
    ExtremeCurve.ToggleButton = toggle

    -- Tombol mata untuk hide/show tombol curve.
    local hide = Instance.new("TextButton")
    hide.Name = "HideButton"
    hide.Size = UDim2.fromOffset(35, 35)
    hide.Position = UDim2.new(0.5, 95, 0, 62)
    hide.BackgroundColor3 = Color3.fromRGB(50, 40, 70)
    ...
    ExtremeCurve.HideButton = hide

    toggle.MouseButton1Click:Connect(function()
        local newValue = not ExtremeCurve.Enabled

        if SetExtremeCurve(newValue) then
            Library:Notify({
                Title = "PC Heavy Curve",
                Description = newValue and "AKTIF" or "NONAKTIF",
                Time = 2
            })
        end
    end)
    ...
    -- Drag untuk tombol utama dan tombol mata.
    ...
    local function updateDrag(input)
        ...
        if dragged then
            ...
            hide.Position = UDim2.new(
                ...
                toggle.Position.X.Offset + 185,
                toggle.Position.Y.Scale,
                toggle.Position.Y.Offset + 2
            )
        end
    end
...
UnivGroup:AddButton({
    Text = "PC Heavy Curve Button",
    Func = function()
        if ExtremeCurve.Gui then
            DestroyExtremeCurveGui()
            Library:Notify({
                Title = "PC Heavy Curve",
                Description = "Button ditutup",
                Time = 2
            })
        else
            CreateExtremeCurveGui()
            Library:Notify({
                Title = "PC Heavy Curve",
                Description = "Button muncul di layar!",
                Time = 2
            })
        end
    end
