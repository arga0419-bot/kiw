...
local function SetExtremeCurve(value)
    ...
    ExtremeCurve.Connection = RunService.RenderStepped:Connect(function()
        ...
        -- Deteksi perubahan arah kamera.
        -- SideDot menggunakan right vector sebagai acuan arah perubahan.
        local sideDot = lastLook:Dot(rightVector)
        if turnDot < 0.95 then
            tiltFactor = sideDot * ExtremeCurve.TiltAmount
        end

        ExtremeCurve.CurrentCFrame = ExtremeCurve.CurrentCFrame:Lerp(
            targetCFrame,
            ExtremeCurve.Smoothness
        )

        currentCamera.CFrame = ExtremeCurve.CurrentCFrame * CFrame.Angles(0, 0, tiltFactor)
        ExtremeCurve.LastLookVector = currentLook
    end)
end

UnivGroup:AddButton({\n    Text = "PC Heavy Curve",\n    Func = function()\n        local newValue = not ExtremeCurve.Enabled\n        SetExtremeCurve(newValue)\n\n        Library:Notify({\n            Title = "PC Heavy Curve",\n            Description = newValue and "AKTIF" or "NONAKTIF",\n            Time = 2\n        })\n    end\n})\n
