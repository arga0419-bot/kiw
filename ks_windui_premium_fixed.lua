...
-- ==================== WINDUI SETUP ====================
-- UI framework migrated from KezodX/Obsidian to WindUI.
-- Feature logic below this point is intentionally kept intact.
...
local function _notify(data)
    if type(data) == "string" then
        WindUI:Notify({Title = "Pandu Hub", Content = data, Duration = 3})
        return
    end
    data = data or {}
    WindUI:Notify({
        Title = data.Title or "Pandu Hub",
        Content = data.Description or data.Content or "",
        Duration = data.Time or data.Duration or 3,
        Icon = data.Icon,
    })
end
Library.Notify = _notify

local function _set_underlying(obj, value)
    if not obj then return end
    local ok = false
    if obj.Set then ok = pcall(function() obj:Set(value) end) end
    if not ok and obj.Select then ok = pcall(function() obj:Select(value) end) end
    if not ok and obj.Update then ok = pcall(function() obj:Update(value) end) end
end

local function _newElement(kind, key, actual, opts)
    local e = {
        Type = kind,
        Value = opts and opts.Default,
        _actual = actual,
        _opts = opts or {},
        _key = key,
        _keyState = false,
    ...
    function e:SetValue(v)
        self.Value = v
        _set_underlying(self._actual, v)
        if self._opts.Callback then pcall(self._opts.Callback, v) end
    end
    function e:Set(v) self:SetValue(v) end
    function e:OnChanged(cb) self._opts.Callback = cb end
    function e:OnClick(cb)
        self._click = cb
    end
    function e:AddKeyPicker(k, o)
        return self._owner:AddKeyPicker(k, o, self)
    end
    function e:AddColorPicker(k, o)
        return self._owner:AddColorPicker(k, o, self)
    end
    return e
end
...
local function _wrapTab(tab)
    ...
    function W:AddLeftGroupbox(title, icon)
        return self:AddRightGroupbox(title, icon)
    end

    function W:Space(columns)
        pcall(function() self._tab:Space({Columns = columns or 1}) end)
    end

    function W:AddRightGroupbox(title, icon)
        local ok, section = pcall(function()
            return self._tab:Section({
                Title = title or "",
                Icon = icon,
                Box = true,
                BoxBorder = true,
        ...
        local G = { _tab = self._tab, _section = section }

        local function target()
            return G._section or G._tab
        end

        function G:AddToggle(key, opts)
            opts = opts or {}
            local actual = target():Toggle({
                Title = opts.Text or key,
                Desc = opts.Tooltip,
                Value = opts.Default == true,
                Type = "Toggle",
                Callback = function(v)
                    local el = Toggles[key]
                    if el then el.Value = v end
                    if opts.Callback then opts.Callback(v) end
                end,
            })
            local el = _newElement("Toggle", key, actual, opts)
            el.Value = opts.Default == true
            el._owner = G
            Toggles[key] = el
            Options[key] = el
            return el
        ...
        function G:AddCheckbox(key, opts)
            opts = opts or {}
            local actual = target():Toggle({
                Title = opts.Text or key,
                Desc = opts.Tooltip,
            ...
            local el = _newElement("Checkbox", key, actual, opts)
            el.Value = opts.Default == true
            el._owner = G
            Toggles[key] = el
            Options[key] = el
        ...
        function G:AddButton(a, b)
            local opts
            if type(a) == "table" then opts = a else opts = {Text = a, Func = b} end
            opts = opts or {}
            local callback = opts.Func or opts.Callback or function() end
            local actual = target():Button({
                Title = opts.Text or opts.Title or "Button",
                Desc = opts.Tooltip or opts.Desc,
                Icon = opts.Icon,
                Callback = callback,
            })
            return {Type="Button", _actual=actual, Highlight=function(self) if actual and actual.Highlight then pcall(function() actual:Highlight() end) end end}
        end

        function G:AddSlider(key, opts)
            opts = opts or {}
            local actual = target():Slider({
                Title = opts.Text or key,
                Desc = opts.Tooltip,
                Step = opts.Rounding and (10 ^ (-opts.Rounding)) or 1,
                Value = {Min = opts.Min or 0, Max = opts.Max or 100, Default = opts.Default or opts.Min or 0},
                Callback = function(v)
                    local el = Options[key]
                    if el then el.Value = v end
                    if opts.Callback then opts.Callback(v) end
                end,
            })
            local el = _newElement("Slider", key, actual, opts)
            el.Value = opts.Default or opts.Min or 0
            el._owner = G
            Options[key] = el
            return el
        end

        function G:AddDropdown(key, opts)
            opts = opts or {}
            local actual = target():Dropdown({
                Title = opts.Text or key,
                Desc = opts.Tooltip,
            ...
            local el = _newElement("Dropdown", key, actual, opts)
            el.Value = opts.Default
            el._owner = G
            function el:SetValues(values)
                self.Values = values
                if self._actual and self._actual.Refresh then pcall(function() self._actual:Refresh(values) end) end
            end
            function el:Select(v)
                self.Value = v
                if self._actual and self._actual.Select then pcall(function() self._actual:Select(v) end) end
            end
            Options[key] = el
            return el
        end

        function G:AddInput(key, opts)
            opts = opts or {}
            local actual = target():Input({
                Title = opts.Text or key,
                Desc = opts.Tooltip,
                Value = opts.Default or "",
                Placeholder = opts.Placeholder,
                Type = opts.Type == "Textarea" and "Textarea" or "Input",
                Callback = function(v)
                    local el = Options[key]
                    if el then el.Value = v end
                    if opts.Callback then opts.Callback(v) end
            ...
            local el = _newElement("Input", key, actual, opts)
            el.Value = opts.Default or ""
            el._owner = G
            Options[key] = el
            return el
        end
        ...
        function G:AddLabel(text)
            local actual = target():Paragraph({Title = tostring(text or "")})
            local el = _newElement("Label", nil, actual, {Default=text})
            el.Value = text
            el._owner = G
            function el:SetText(v) self.Value = v end
            return el
        end

        function G:AddDivider()
            pcall(function() target():Divider() end)
        end

        function G:AddKeyPicker(key, opts, linked)
            opts = opts or {}
            local defaultKey = opts.Default or "None"
            local active = false
            local isMenuKey = key == "MenuKeybind"
            local actual = target():Keybind({
                Title = opts.Text or key,
                Desc = opts.Tooltip,
                Value = defaultKey,
                HoldToInteract = opts.Mode == "Hold",
                CallOnChange = isMenuKey,
                Callback = function(v)
                    if isMenuKey and type(v) == "string" then
                        local kc = Enum.KeyCode[v]
                        if kc then pcall(function() Window:SetToggleKey(kc) end) end
                        return
                    end
                    ...
                    if cb then cb(active)
                    elseif linked and linked._opts and linked._opts.Callback then linked._opts.Callback(active) end
                    if linked then
                        linked.Value = active
                        if linked._actual and linked._actual.Set then pcall(function() linked._actual:Set(active) end) end
                    end
                end,
            })
            local kp = _newElement("KeyPicker", key, actual, opts)
            kp.Value = defaultKey
            kp._owner = G
        ...
        function G:AddColorPicker(key, opts, linked)
            opts = opts or {}
            local actual = target():Colorpicker({
                Title = opts.Title or opts.Text or key,
                Desc = opts.Tooltip,
            ...
            local el = _newElement("ColorPicker", key, actual, opts)
            el.Value = opts.Default or Color3.new(1,1,1)
            el._owner = G
            Options[key] = el
            return el
        end
...
local Window = WindUI:CreateWindow({
    Title = "Pandu Hub",
    Author = "Violence District • Premium",
    Folder = "Pandu",
    Icon = "solar:gamepad-bold",
    IconThemed = true,
    NewElements = true,
    Size = UDim2.fromOffset(680, 700),
    Theme = "Dark",
    Resizable = true,
    SideBarWidth = 205,
    HideSearchBar = false,
...
function Library:CreateWindow(_) return Window end
function Library:Unload() pcall(function() Window:Destroy() end) end
Library.ToggleKeybind = nil
Library.AddDraggableLabel = function(_, text) return {Text=text} end

pcall(function() Window:SetToggleKey(Enum.KeyCode.RightShift) end)

local TabsUI = {
    Main = _wrapTab(Window:Tab({Title="Pandu", Desc="Dashboard & utility", Icon="solar:home-2-bold", IconShape="Square", Border=true})),
    Survivor = _wrapTab(Window:Tab({Title="Survivor", Desc="Survivor tools", Icon="solar:user-bold", IconShape="Square", Border=true})),
    Killer = _wrapTab(Window:Tab({Title="Killer", Desc="Killer tools", Icon="solar:skull-bold", IconShape="Square", Border=true})),
    Esp = _wrapTab(Window:Tab({Title="ESP", Desc="Visual & world", Icon="solar:eye-bold", IconShape="Square", Border=true})),
    Teleport = _wrapTab(Window:Tab({Title="Teleport", Desc="Players & map", Icon="solar:map-point-bold", IconShape="Square", Border=true})),
    ["UI Settings"] = _wrapTab(Window:Tab({Title="UI Settings", Desc="Interface settings", Icon="solar:settings-bold", IconShape="Square", Border=true})),
}

-- Lightweight compatibility for the old config manager calls at the end.
...
-- Premium dashboard: visual-only additions, feature logic remains unchanged.
pcall(function()
    local Dash = TabsUI.Main._tab:Section({
        Title = "PANDU HUB",
        Desc = "Violence District • Premium Control Center",
    ...
    Dash:Paragraph({
        Title = "Ready",
        Content = "All core features are loaded below. Use the sidebar to jump between Survivor, Killer, ESP and Teleport tools.",
    })
end)
    ...
    mt.__namecall = newcclosure(function(self, ...)
        ...
        then
            ...
            if AimConfig.Aim_SilentVeilV2 and not isFiringSpear then
                -- Ambil argumen FireServer dari vararg __namecall dengan aman
                local args = {...}
                local lookVec = args[1]
                local speed = args[2]
...
-- ==================== WINDUI SETTINGS ====================
-- WindUI handles the visual settings; the legacy Obsidian save/theme calls are intentionally removed.
local SettingsSection = TabsUI["UI Settings"]:AddRightGroupbox("UI Settings", "settings")
SettingsSection:AddLabel("Pandu Hub • WindUI")
SettingsSection:AddLabel("UI berhasil dimigrasikan dari KezodX/Obsidian.")
SettingsSection:AddButton({Text = "Close UI", Func = function() pcall(function() Window:Destroy() end) end})
Library.CantDragForced = false
Library:Notify({ Title = "Pandu Hub", Description = "WindUI berhasil dimuat!", Time = 3 })
