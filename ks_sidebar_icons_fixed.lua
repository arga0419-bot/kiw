...
-- Fallback untuk Lucide icons apabila provider icon utama gagal dimuat.
do
    task.spawn(function()
        ...
        if not ok or not IconModule or not IconModule.GetAsset then
            return
        end

        local iconNames = {
            "house", "users", "swords", "eye", "map-pin", "settings"
        }

        for index, name in ipairs(iconNames) do
            local tabInfo = Library.TabButtons and Library.TabButtons[index]
            if tabInfo and tabInfo.Label and tabInfo.Label.Parent then
                local tabButton = tabInfo.Label.Parent
                local success, icon = pcall(IconModule.GetAsset, name)

                if success and icon and icon.Url then
                    local image = tabInfo.Icon
                    if not image or not image.Parent then
                        image = Instance.new("ImageLabel")
                        image.Name = "SidebarIcon"
