...
-- ==================== PC ULTRA CURVE ====================
-- Tombol menu hanya membuat tombol kontrol curve muncul di layar.
local PCUltraCurve = {
    Gui = nil,
    ToggleButton = nil,
    HideButton = nil,
...
local function PCUltraCurveUpdateButton()
    if not PCUltraCurve.ToggleButton then return end
    if PCUltraCurve.Enabled then
        PCUltraCurve.ToggleButton.Text = "PC Ultra Curve: ON"
        PCUltraCurve.ToggleButton.BackgroundColor3 = Color3.fromRGB(255, 100, 150)
    else
        PCUltraCurve.ToggleButton.Text = "PC Ultra Curve: OFF"
        PCUltraCurve.ToggleButton.BackgroundColor3 = Color3.fromRGB(30, 24, 45)
    end
end
...
local function PCUltraCurveStart()
    ...
    PCUltraCurve.CurrentCFrame = cam.CFrame
    PCUltraCurve.LastLookVector = cam.CFrame.LookVector
    PCUltraCurve.ArcVelocity = Vector3.new(0, 0, 0)
    ...
    PCUltraCurve.Connection = RunService.RenderStepped:Connect(function()
        ...
        if turnDot < 0.98 then
            tiltFactor = math.clamp(
                turnAmount * PCUltraCurve.TiltAmount * 4,
            ...
            PCUltraCurve.ArcVelocity =
                ...
                + (currentLook * (turnAmount * PCUltraCurve.Overshoot))
        end
...
local function PCUltraCurveCreateGui()
    ...
    local gui = Instance.new("ScreenGui")
    gui.Name = "PanduHub_PCUltraCurve"
    gui.ResetOnSpawn = false
    gui.IgnoreGuiInset = true
    gui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
    gui.Parent = parent
    ...
    toggle.Position = UDim2.new(0.5, -90, 0, 10)
    toggle.BackgroundColor3 = Color3.fromRGB(30, 24, 45)
    toggle.TextColor3 = Color3.fromRGB(255, 255, 255)
    toggle.Text = "PC Ultra Curve: OFF"
    toggle.TextSize = 14
    ...
    hide.Position = UDim2.new(0.5, 95, 0, 12)
    ...
    PCUltraCurve.ToggleButton = toggle
    PCUltraCurve.HideButton = hide
    ...
    hide.MouseButton1Click:Connect(function()
        ...
        hide.Text = toggle.Visible and "X" or ">"
    end)
...
FakePerksGroup:AddButton({
    ...
    Func = function()
        ...
        Library:Notify({
            ...
            Description = "Tombol curve muncul di layar",
            Time = 2
        })
    end
})
